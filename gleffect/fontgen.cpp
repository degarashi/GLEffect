...
Face::Face(const SPString& name, const PowSize& sfcSize, CCoreID cid, FontChMap& m):
...
const CharPos* Face::getCharPos(char32_t c) {
	...
	if(res.second.width() <= 0) {
		...
		float invW = spn::_sseRcp22Bit(static_cast<float>(sz.width)),
			invH = spn::_sseRcp22Bit(static_cast<float>(sz.height));

		float h = res.second.height();
		const auto& uvr = lraw.rect;
		cp.uv = RectF(uvr.x0 * invW,
						uvr.x1 * invW,
						uvr.y0 * invH,
						(uvr.y0+h) * invH);
	}
...
void TextObj::_init(Face& face) {
	...
	float dt = spn::_sseRcp22Bit(_text.length()),
		t = 0;
	for(auto& c : _text) {
...
const SizeF& TextObj::getSize() const {
	return _rectSize;
}
...
FontGen::FontGen(const PowSize& sfcSize): _sfcSize(sfcSize) {}
